...
export default function SettingsTab({
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      setImportMsg({
        ok,
        text: t(ok ? "settings.importSuccess" : "settings.importFail", locale),
      });
    ...
    <div className="animate-tab-enter space-y-5">
      ...
      <div className="bg-card rounded-2xl p-5 border border-card space-y-3">
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl text-sm bg-subtle text-muted hover:opacity-80 transition-opacity"
        >
        ...
        {importMsg && (
          <p className={`text-xs text-center ${importMsg.ok ? "text-emerald-500" : "text-red-400"}`}>
            {importMsg.text}
          </p>
        )}
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl text-sm text-red-400 bg-subtle hover:opacity-80 transition-opacity"
        >
